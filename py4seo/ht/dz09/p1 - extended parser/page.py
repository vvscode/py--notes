...
class Page:
    ...
    @property
    def images(self):
        return [urljoin(self.normalized_url, element.attrs['src']) for element in self.response.html.find(f'img')]

    def get_titles(self, level = 1):
        return [element.text for element in self.response.html.find(f'h{level}')]